...
void main()
{
  vec4 tc = vec4(0.0);
    ...
    vec2 uv = gl_FragCoord.xy / vec2(640.0/2,360/2.0);
    ...
    for (int i=1; i<3; i++)
    {
      tc += texture2D(sceneTex, uv + vec2(offset[i])/rt_w, 0.0).rgba *texture2D(sceneTex, uv + vec2(offset[i])/rt_w, 0.0).a
      *weight[i] ;
            
     sum +=weight[i]*texture2D(sceneTex, uv + vec2(offset[i])/rt_w, 0.0).a;
      tc += texture2D(sceneTex, uv - vec2(offset[i])/rt_w, 0.0).rgba*texture2D(sceneTex, uv - vec2(offset[i])/rt_w, 0.0).a
              *weight[i];
     sum +=weight[i]*texture2D(sceneTex, uv - vec2(offset[i])/rt_w, 0.0).a;
    }
...
void main()
{
  vec4 tc = vec4(0.0);
    ...
    vec2 uv = gl_FragCoord.xy / vec2(640.0/8,360/8.0);
