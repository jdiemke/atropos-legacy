...
//sdTorus(rotateY(rotateX(position+vec3(0.0,0.0,-6.0) ,time*3.0),time*3.0), vec2(4.0,1.0))- 0.2* perlin(rotateY(rotateX(position+vec3(0.0,0.0,-6.0) ,time*1.0),time*3.0)*1.8);

//spikeball(position) ;
	float c =  udRoundBox(position+vec3(0,0,-16), vec3(25.6,15.6,0.6), 0.2 );
//udRoundBox(rotateY(rotateX(position+vec3(0.0,0.0,-6.0),time*3.0),time*3.0)
//, vec3(0.7,0.7+0.7*abs(sin(time*4.5)),0.7), 0.4) + 0.4* perlin(position*1.0);
...
{	
	...
	for(int i=0; i < 64; i++) {
		...
		if(abs(distance) < 0.008) {
			...
			//sss
			float sss = 1.0 -computeAO(position,viewDirection)*6.0;
...
color =mix( color,vec4(1.0, 0.5,0.1,1.0),(1.0-sss));
			// interation glow
			color += vec4(vec3(1.0, 0.5,0.1)*pow(float(i)/64.0*1.7, 2.0) *1.0,1.0);
