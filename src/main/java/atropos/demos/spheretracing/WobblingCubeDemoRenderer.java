...
public class WobblingCubeDemoRenderer extends AtroposDefaultRenderer {
	...
	public void init(GLAutoDrawable drawable) {
		...
		vs.setShaderSource(gl, new ShaderSource(new File("shaders/spheretracing/wobblingCube.vs")));
		vs.compileShader(gl);
		System.err.println(vs.getInfoLog(gl));

		FragmentShader fs = new FragmentShader(gl);
		fs.setShaderSource(gl, new ShaderSource(new File("shaders/spheretracing/wobblingCube.fs")));
		fs.compileShader(gl);
	...
	public void display(GLAutoDrawable drawable) {
		...
		time.set(gl, elapsedTime/1000.0f*0.8f);
