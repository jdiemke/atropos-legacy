...
public class InterferenceDemoRenderer extends AtroposDefaultRenderer {
...
//		System.err.println(fs.getInfoLog(gl));
		...
		VertexShader vs = new VertexShader(gl);
		vs.setShaderSource(gl, new ShaderSource(new File("shaders/interferenceShader.vs")));
		vs.compileShader(gl);
		System.err.println(vs.getInfoLog(gl));

		FragmentShader fs = new FragmentShader(gl);
		fs.setShaderSource(gl, new ShaderSource(new File("shaders/interferenceShader.fs")));
		fs.compileShader(gl);
		System.err.println(fs.getInfoLog(gl));
	 ...
	 prog = new ShaderProgram(gl);
		...
		try {
			player.loadModule(new File("music/xm/DUBMOOD - Paradox-crackchip 2.xm"));
		} catch (IOException e) {
