...
public class RotoZoomDemoRenderer extends AtroposDefaultRenderer {
...
//		System.err.println(fs.getInfoLog(gl));
		...
		VertexShader vs = new VertexShader(gl);
		vs.setShaderSource(gl, new ShaderSource(new File("./shaders/rotozoomShader.vs")));
		vs.compileShader(gl);
		System.err.println(vs.getInfoLog(gl));

		FragmentShader fs = new FragmentShader(gl);
		fs.setShaderSource(gl, new ShaderSource(new File("./shaders/rotozoomShader.fs")));
		fs.compileShader(gl);
		//System.err.println(fs.getInfoLog(gl));
	 ...
	 prog = new ShaderProgram(gl);
		...
		texture = load(gl,"./textures/rick/melon.png");
		texture = load(gl, "./textures/rick/meanman.png");
		texture = load(gl, "./textures/rick/gns.png");
...
//	gl.glEnd();
	...
	void textWriter(GL2 gl) {
		...
		for(int column=0; column < page.length; column++) {
			...
			pos=640/2-(width/2) +(int)((640)*(1-CosineInterpolate(0+column*400, 2000+column*400, localTime)))
			- (int)((640)*(CosineInterpolate(0+( column)*400+(12000-2000-400*(page.length-1)), 2000+(column)*400+(12000-2000-400*(page.length-1)), localTime)));
		for(int i=0; i < page[column].length(); i++) {
	
			
			 drawTile(gl,(int)(page[column].charAt(i)-(int)' '),(int)( pos),(int)(posy+8*2*column), 1.0f);
	 ...
	 Texture load (GL gl,String filename)
	   {
		 System.out.println(filename);
	      ...
	      try
	      {
	         ...
	         // texture = TextureIO.newTexture(texture.getTextureObject(gl), GL.GL_TEXTURE_2D,
	        	//	  texture.getWidth(),texture.getHeight(), texture.getWidth(),texture.getHeight(), true);
...
texture.bind(gl);
		...
		gl.glVertex2f(640,360-0);

		gl.glVertex2f(0,360-0);
		gl.glEnd();
		prog.deactivate(gl);
